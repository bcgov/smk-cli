            ...
            <div class="row" style="padding: 0px; margin: 0px;">
                <div id="allTools" class="col s6" style="height: calc(100vh - 282px);">
                    ...
                    <ul id="catalogLayersList" class="collection" style="height: 100%; overflow: auto;">
                        <tool-component v-for="tool in config.tools"
                                        v-bind:tool="tool"
                                        v-bind:key="tool.type + '-' + tool.instance">
                        </tool-component>
                    </ul>
                </div>
                <div id="editPanel" class="col s6" style="height: calc(100vh - 282px); margin-top: 74px;">
...
Vue.component('tool-component',
{
    props: ['tool'],
    template:   
        `<li v-if="tool.type !=='query'" class="collection-item avatar">
            <span class="title">{{tool.type}} tool</span>
            <p>
                <label>
                    <input v-bind:name="'' + tool.type + '-' + tool.instance + '-operatorGroup'" type="radio" value="true" v-model="tool.enabled"/>
                    <span>Enabled</span>
                </label>
                <label>
                    <input v-bind:name="'' + tool.type + '-' + tool.instance + '-operatorGroup'" type="radio" value="false" v-model="tool.enabled"/>
                    <span>Disabled</span>
                </label>
                <br />
            </p>
            <a href="#!" v-bind:onclick="'editTool(\\'' + tool.type + '\\')'" class="secondary-content"><i class="material-icons black-text">edit</i></a>
        </li>`
});
