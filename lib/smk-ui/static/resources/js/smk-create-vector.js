	...
	else
	{
		switch($('#vectorType').val()) 
		{
			...
			case 'shape':
				...
				$.ajax
				({
					url: serviceUrl + "ProcessShape",
					type: "post",
					data: documentData,
					cache: false,
					contentType: false,
					processData: false,
					success: function (convertedShape)
					{
						addLayerToConfig(layer);
						saveGeoJsonFile(layer.id, convertedShape);
					},
					error: function (status)
					{
						// error handler
						M.toast({ html: 'Error converting Shapefile. Error: ' + status.responseText});
						console.log('Error converting Shapefile. Error: ' + status.responseText);
					}
				});
			break;
			case 'pgdb':
